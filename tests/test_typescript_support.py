...
class TestTypeScriptSupport:
    ...
    async def test_mixed_directory_analysis(self, autodoc, test_ts_project_path):
        ...
        if not TYPESCRIPT_AVAILABLE or not autodoc.ts_analyzer or not autodoc.ts_analyzer.is_available():
...
""")
    ...
    async def test_typescript_only_analysis(self, autodoc, test_ts_project_path):
        ...
        assert len(found_entities) > 0, f"Expected some entities from {expected_entities}, but found {entity_names}"
    ...
    def test_internal_service_classification(self, autodoc):
        ...
        mock_entity.code = "class UserService { async createUser() { return this.database.save(); } }"
